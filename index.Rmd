...
- https://michabohealthscience.github.io/omics-grouping-training/
...
- https://github.com/michabohealthscience/omics-grouping-training/
...
1. [Data](1_data.html) - Where we introduce the example omics dataset we are using from the Cefic MATCHING project [@viant2024demonstrating] 
2. [Data processing](2_data_processing.html) - Where we read in the processed dataset 
3. [Quality assessments](3_qa.html) - A brief note on quality assessments 
4. [Differentially abundant molecules - statistical analysis](4_stats.html) - Where we assess the bioactivity of the dataset and generate the input for the grouping 
5. [Grouping](5_grouping.html)  - Where we use metrics generated from the univariate statistical analysis from the prior step to perform grouping 
...
R (and R Studio) should already be pre-installed on all the trainees computers. 

We will work through the tutorial by copying the commands into an R Script and running the commands within R Studio.
...
If you have already done so, please run the following command to install the relevant packages needed for the training. 


```{r package install, eval=FALSE}
install.packages(c('pvclust', 'ggplot2', 'ggfortify', 'dplyr', 'reshape2', 'tidyr'))
```
